...
class ViewController: NSViewController, NSTextViewDelegate {
    ...
    @IBAction func saveOutput(sender: AnyObject) {
        ...
        let fileManager = NSFileManager.defaultManager()
            ...
            if (fileManager.fileExistsAtPath(fileURL.path!)) {
                ...
                warningLabel.stringValue = "File already exists - no action"
            } else {
                saveText(output, path: fileURL.path!)
                print("File saved")
    ...
    @IBAction func searchPress(sender: AnyObject) {
        ...
        for var n = 0; n < myFilteredRecs.count-1; n++ {
